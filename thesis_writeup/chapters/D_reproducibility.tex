...
\begin{verbatim}
  ...
  scripts/                   top-level entry points
    ...
    run_llm_warmstart.py     design agent driver
...
\begin{itemize}
  ...
  \item \textbf{Phase 4 agent}: \texttt{anthropic==0.96.0},
...
\begin{verbatim}
  ...
  # (optional) populate your Anthropic API key for agent re-runs:
...
agent sampling) uses a caller-supplied integer seed. Across
...
\subsection{Ch 4.6 --- agent warm-start}
\begin{verbatim}
...
cache files are committed to the repository so the agent ablation
...
\begin{table}[h]
  ...
  \caption{Artefact manifest. Figures are regenerated on demand
    (git-ignored); CSV summaries, agent cache JSONs, and run
...
\begin{itemize}
  ...
  \item \texttt{tests/test\_llm\_*.py}: 11 tests --- cache I/O
    shape checks, agent warm-start determinism given cache.
\end{itemize}
